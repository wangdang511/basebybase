...
/*#######################################################################>>>>>
* Purpose: This is an ABSTRACT class for all the pairwise graphs that can be plotted in BASE-BY-BASE.
* Written: Winter 2009
* Edited: Summer 2015
* #######################################################################>>>>>*/
public abstract class AbstractPairwiseGraph extends JFrame {
    protected AbstractPairwiseGraph theFrame = this;
    ...
    * #######################################################################>>>>>*/
    ...
    /*#######################################################################>>>>>
    * Purpose: Create an AbstractPairwiseGraph object.
    * Written: Winter 2009
    * Edited: Summer 2015
    * #######################################################################>>>>>*/
    public AbstractPairwiseGraph(FeaturedSequence[] seqs) {
        setSeqs(seqs);
        backupSeqs = getSeqs();
        nSeqs = seqs.length;
    ...
    * #######################################################################>>>>>*/
    private JPanel generateCtrlPanel() {
        ...
        String[] options = new String[nSeqs];
        for (int i = 0; i < nSeqs; i++)
            options[i] = seqs[i].getName();
        JComboBox selectRefVirus = new JComboBox(options);
        selectRefVirus.setMaximumSize(selectRefVirus.getPreferredSize());
        selectRefVirus.setSelectedIndex(0);
        selectRefVirus.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                JComboBox cb = (JComboBox) e.getSource();
                refSeqIndex = cb.getSelectedIndex();
            }
        });
        //Delete Gap checkbox
        ...
        cPanel.add(new JLabel("Select Reference Sequence: "));
        cPanel.add(selectRefVirus);
    ...
    * #######################################################################>>>>>*/
    ...
    /*#######################################################################>>>>>
    * Purpose: Get current reference sequence index.
    * Param: New reference sequence index
    * Written: Winter 2009
    * #######################################################################>>>>>*/
    protected void setRefSeqIndex(int index) {
        refSeqIndex = index;
    }

    /*#######################################################################>>>>>
    * Purpose: Get current reference sequence index.
    * Return: Index of the reference sequence.
    * Written: Winter 2009
    * #######################################################################>>>>>*/
    protected int getRefSeqIndex() {
        return refSeqIndex;
    }
