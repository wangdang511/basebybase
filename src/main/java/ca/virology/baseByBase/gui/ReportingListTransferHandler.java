...
public class ReportingListTransferHandler extends TransferHandler {
    ...
    public String blankDNDListing = ""; //used to always have an element even on an empty list
    public boolean ignoreAtEnd = false;
    ...
    public boolean importData(JComponent c, Transferable t) {
        ...
        if (listModel.size() > 1) {
            for (int i = 0; i < listModel.size(); i++) {
                if (listModel.elementAt(i) == blankDNDListing) {
                    ignoreAtEnd = true;
                    listModel.remove(i);
                }
            }
    ...
    protected Transferable createTransferable(JComponent c) {
        //caity - getSelectedValues is deprecated -> switch to getSelectedValuesList  * Function not used, therefore not tested
...
//        return null;
        if (c instanceof JList) {
            source = (JList) c;
            indices = source.getSelectedIndices();
            List<Object> list = source.getSelectedValuesList();
            if (list == null || list.size() == 0) {
                return null;
            }
            ArrayList<String> alist = new ArrayList<>(list.size());
            for (Object o : list) {
                if (o == null) {
                    alist.add("");
                } else {
